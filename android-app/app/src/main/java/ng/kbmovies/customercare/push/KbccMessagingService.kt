...
class KbccMessagingService:FirebaseMessagingService(){
    override fun onNewToken(token:String){super.onNewToken(token);CoroutineScope(Dispatchers.IO).launch{runCatching{(application as CareApp).repository.registerDeviceToken(token)}}}
